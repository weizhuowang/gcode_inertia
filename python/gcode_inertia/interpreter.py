...
class GCodeInterpreter:
    ...
    def _next_coord(self, current: float, value: Optional[float], relative: bool) -> float:
        """Calculate next coordinate based on mode."""
        if value is None:
            return current
        return current + value if relative else value
    
    def process_command(self, cmd_dict: Dict[str, Union[str, float]]) -> Optional[Segment]:
    ...
    def _handle_motion(self, cmd: str, params: Dict[str, Union[str, float]]) -> Optional[Segment]:
...
def parse_segments(gcode_str: str, filament_diameter: float = 1.75, 
                  filament_density: float = 1.25) -> List[Segment]:
    ...
    Args:
        ...
        filament_density: Filament density in g/cm³ (default: 1.25 for PLA)
